...
const Confirmed: FC = () => {
    ...
    const description = (commitTxId && source_network) && <p><span>Transaction ID:</span> <Link target="_blank" className="underline hover:no-underline" href={(source_network as any)?.transactionExplorerTemplate.replace('{0}', commitTxId)}>{shortenAddress(commitTxId)}</Link></p>
...
const AssetsReady: FC = () => {
    ...
    const description = (lpLockTx && destination_network) ? <p><span>Transaction ID:</span> <Link className="underline hover:no-underline" target="_blank" href={(destination_network as any)?.transactionExplorerTemplate.replace('{0}', lpLockTx?.hash)}>{shortenAddress(lpLockTx.hash)}</Link></p> : <div className="h-3 w-10 bg-gray-400 animate-pulse rounded" />
...
const SignAndConfirm: FC = () => {
    ...
    const description = (addLockSigTx && source_network) ? <p><span>Transaction ID:</span> <Link className="underline hover:no-underline" target="_blank" href={(source_network as any)?.transactionExplorerTemplate.replace('{0}', addLockSigTx?.hash)}>{shortenAddress(addLockSigTx.hash)}</Link></p> : <div className="h-3 w-10 bg-gray-400 animate-pulse rounded" />
