    ...
    return (
        <div ref={topmostRef}>
            ...
            <motion.div
                ...
                ref={mobileModalRef}
                animate={controls}
                className={`max-h-full overflow-y-hidden group ${position} inset-x-0 bottom-0 z-40 w-full ${height != 'full' ? 'rounded-t-2xl' : 'rounded-t-containerRoundness'}  bg-secondary-900 ${wrapperHeightClass} ${className} shadow-lg`}
                initial={{ y: "20%" }}
                exit={{ y: "100%" }}
                transition={transitionProps}
            ...
            >
                <div className='py-3 overflow-y-auto flex flex-col h-full z-40 bg-secondary-900 pb-6'>
