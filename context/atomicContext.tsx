...
export function AtomicProvider({ children }) {
    ...
    const { data } = useSWR<ApiResponse<CommitFromApi>>((parsedCommitId && commitFromApi?.transactions.length !== 3) ? `${url}/api/swaps/${parsedCommitId}` : null, fetcher, { refreshInterval: 5000 })
